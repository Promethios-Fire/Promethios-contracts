...
const toUFrgDenomination = (fire: string): BigNumber =>
  ethers.utils.parseUnits(fire, DECIMALS)
...
describe('UFragments:Initialization', () => {
  ...
  it('should set detailed ERC20 parameters', async function () {
    expect(await uFragments.name()).to.eq('Promethios')
    expect(await uFragments.symbol()).to.eq('FIRE')
...
describe('UFragments:Mint/Burn', function () {
  let A: Signer, B: Signer, C: Signer
  ...
  describe('mint()', () => {
    it('should mint the specified amount to the target address', async () => {
      const initialTotalSupply = await uFragments.totalSupply()
      const amountToMint = ethers.BigNumber.from(10).pow(6)
      const user1 = await A.getAddress()

      const receipt = await uFragments
        .connect(deployer)
        .mint(user1, amountToMint)
      expect(receipt)
        .to.emit(uFragments, 'Mint')
        .withArgs(user1, amountToMint, initialTotalSupply.add(amountToMint))

      const newTotalSupply = await uFragments.totalSupply()
      const user1Balance = await uFragments.balanceOf(user1)

      expect(user1Balance).to.be.equal(amountToMint)
      expect(newTotalSupply).to.be.equal(initialTotalSupply.add(amountToMint))
    })

    it('should revert if mint called by non-owner', async () => {
      const amountToMint = ethers.BigNumber.from(10).pow(6)
      const user1 = await A.getAddress()

      await expect(uFragments.connect(B).mint(user1, amountToMint)).to.be
        .reverted
    })

    it('should burn the specified amount to the target address', async () => {
      const amountToBurn = ethers.BigNumber.from(10).pow(6)
      const user1 = await A.getAddress()
      const initialTotalSupply = await uFragments.totalSupply()
      const initialUser1Balance = await uFragments.balanceOf(user1)

      const receipt = await uFragments
        .connect(deployer)
        .burn(user1, amountToBurn)
      expect(receipt)
        .to.emit(uFragments, 'Burn')
        .withArgs(user1, amountToBurn, initialTotalSupply.sub(amountToBurn))

      const newTotalSupply = await uFragments.totalSupply()
      const finalUser1Balance = await uFragments.balanceOf(user1)

      expect(finalUser1Balance).to.be.equal(
        initialUser1Balance.sub(amountToBurn),
      )
      expect(newTotalSupply).to.be.equal(initialTotalSupply.sub(amountToBurn))
    })

    it('should revert if burn more than balance', async () => {
      const user1 = await A.getAddress()
      let amountToBurn = await uFragments.balanceOf(user1)
      amountToBurn = amountToBurn.add(ethers.BigNumber.from(10).pow(6))

      await expect(
        uFragments.connect(deployer).burn(user1, amountToBurn),
      ).to.be.revertedWith('Insufficient balance to burn')
    })

    it('should revert if burn called by non-owner', async () => {
      const amountToMint = ethers.BigNumber.from(10).pow(6)
      const user1 = await A.getAddress()

      await expect(uFragments.connect(B).burn(user1, amountToMint)).to.be
        .reverted
    })
  })
})
